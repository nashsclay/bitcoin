...
//! -mintxfee default
static const CAmount DEFAULT_TRANSACTION_MINFEE = 1 * COIN; // change this when adjusting TX fees (satoshis/kB)
//! minimum recommended increment for BIP 125 replacement txs
static const CAmount WALLET_INCREMENTAL_RELAY_FEE = 5 * COIN; // 5 * DEFAULT_TRANSACTION_MINFEE
...
static const unsigned int DEFAULT_TX_CONFIRM_TARGET = 2; // try to get the TX included in the next 2 blocks
//! -walletrbf default
static const bool DEFAULT_WALLET_RBF = false;
static const bool DEFAULT_WALLETBROADCAST = true;
static const bool DEFAULT_DISABLE_WALLET = false;
//! -maxtxfee default
constexpr CAmount DEFAULT_TRANSACTION_MAXFEE{10000 * COIN}; // 10000 * DEFAULT_TRANSACTION_MINFEE
//! Discourage users to set fees higher than this amount (in satoshis) per kB
constexpr CAmount HIGH_TX_FEE_PER_KB{1000 * COIN}; // 1000 * DEFAULT_TRANSACTION_MINFEE
...
public:
    ...
    bool SubmitMemoryPoolAndRelay(std::string& err_string, bool relay, interfaces::Chain::Lock& locked_chain, const std::string& strCommand=NetMsgType::TX);
    ...
    bool IsInMainChain(interfaces::Chain::Lock& locked_chain, bool enableIX = true) const { return GetDepthInMainChain(locked_chain, enableIX) > 0; }
...
private:
    ...
    //! if fOnlyMintingAllowed is true, only minting should be allowed in unlocked wallet
    bool fOnlyMintingAllowed = false;
    ...
    bool Unlock(const CKeyingMaterial& vMasterKeyIn, bool accept_no_keys = false, bool fForMintingOnly = false);
    ...
    std::set<COutPoint> setWalletUTXO GUARDED_BY(cs_wallet);
...
public:
    ...
    bool SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet,
                    const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used, AvailableCoinsType nCoinType=ALL_COINS, bool fUseInstantSend = true) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    // Stake Settings
    uint64_t nStakeSplitThreshold GUARDED_BY(cs_wallet) = 10000;
    ...
    // if CWallet is fully locked so that no operations requiring access
    // to private keys are possible:
    //      IsLocked(true)
    // or if CWallet's private keys are available for minting only:
    //      !IsLocked(true) && IsLocked()
    // or if they are available for everything:
    //      !IsLocked()
    bool IsLocked(bool fForMinting = false) const;
    bool Lock(bool fAllowMinting = false);
     ...
     */
    void AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe = true, const CCoinControl* coinControl = nullptr, const CAmount& nMinimumAmount = 1, const CAmount& nMaximumAmount = MAX_MONEY, const CAmount& nMinimumSumAmount = MAX_MONEY, const uint64_t nMaximumCount = 0, AvailableCoinsType nCoinType=ALL_COINS, bool fUseInstantSend = false) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    bool IsDenominated(const COutPoint& outpoint) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    bool Unlock(const SecureString& strWalletPassphrase, bool accept_no_keys = false, bool fForMintingOnly = false);
    ...
    bool GetBudgetSystemCollateralTX(CTransactionRef& tx, uint256 hash, CAmount amount, bool fUseInstantSend) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, CAmount amount, bool fUseInstantSend) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    bool CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, CValidationState& state, const std::string& strCommand=NetMsgType::TX);
