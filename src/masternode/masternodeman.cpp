...
void CMasternodeMan::Check()
{
    ...
    LogPrint(BCLog::MASTERNODE, "CMasternodeMan::Check -- nLastSentinelPingTime=%d, IsSentinelPingActive()=%d\n", nLastSentinelPingTime, IsSentinelPingActive());
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
        ...
        if (CheckMnbAndUpdateMasternodeList(pfrom, mnb, nDos, connman)) {
            ...
            addr.emplace_back(mnb.addr, NODE_NETWORK);
            connman.AddNewAddresses(addr, pfrom->addr, 2*60*60);
        } else if(nDos > 0) {
            //LOCK(cs_main);
        ...
        CInv inv(MSG_MASTERNODE_PING, nHash);
        EraseInv(inv, pfrom);
...
void CMasternodeMan::WarnMasternodeDaemonUpdates()
{
    ...
    // trigger GUI update
    //uiInterface.NotifyAlertChanged(SerializeHash(strWarning), CT_NEW); TODO
    // trigger cmd-line notification
    //CAlert::Notify(strWarning); REMOVE
