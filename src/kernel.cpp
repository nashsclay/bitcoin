...
bool CheckStakeKernelHash(const unsigned int& nBits, const CBlockIndex* pindexPrev, const CBlockIndex* pindexFrom, const CTxOut& prevTxOut, const unsigned int& nTimeTxPrev, const COutPoint& prevout, unsigned int& nTimeTx, unsigned int nHashDrift, bool fCheck, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    const int64_t nStakeMinAge = nHeightCurrent >= params.nMandatoryUpgradeBlock[1] ? params.nStakeMinAge[1] : params.nStakeMinAge[0];
    const int nStakeMinDepth = nHeightCurrent >= params.nMandatoryUpgradeBlock[0] ? params.nStakeMinDepth[1] : params.nStakeMinDepth[0];
    ...
    if (fCheck) {
        ...
        return stakeTargetHit(hashProofOfStake, nValueIn, bnTargetPerCoinDay, nHeightCurrent >= params.nMandatoryUpgradeBlock[1] || nHeightCurrent < params.nMandatoryUpgradeBlock[0]);
    }
...
bool CheckProofOfStake(BlockValidationState& state, const CCoinsViewCache& view, const CBlockIndex* pindexPrev, const CTransactionRef& tx, const unsigned int& nBits, unsigned int nTimeTx, uint256& hashProofOfStake)
{
    ...
    if (!CheckStakeKernelHash(nBits, pindexPrev, pindexFrom, coin.out, coin.nTime, prevout, nTimeTx, nInterval, true, hashProofOfStake, gArgs.GetBoolArg("-debug", false)))
